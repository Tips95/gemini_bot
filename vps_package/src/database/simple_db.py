...
class SimpleDatabase:
    ...
    def init_database(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_image_generations_user_id ON image_generations(user_id)')
            
            conn.commit()
            conn.close()
            logger.success("Simple database initialized successfully with indexes")
    ...
    async def check_subscription(self, telegram_id: int) -> bool:
        ...
        try:
            ...
            # Если не найдено в users, проверяем в subscriptions
            cursor.execute('''
                SELECT expires_at 
                FROM subscriptions 
            ...
            logger.warning(f"No active subscription for user {telegram_id}")
            return False
    ...
    async def create_subscription(self, telegram_id: int, plan_name: str, price: int, duration_days: int, payment_id: str = None) -> bool:
        ...
        try:
            ...
            # Деактивируем старые подписки в обеих таблицах
            cursor.execute('UPDATE users SET subscription_active = FALSE WHERE telegram_id = ?', (telegram_id,))
            cursor.execute('UPDATE subscriptions SET is_active = FALSE WHERE user_id = ?', (telegram_id,))
    ...
    def backup_database(self):
        ...
        try:
            ...
            backup_path = f"backups/bot_subscriptions_{timestamp}.db"
